...
func (sq *SubscriptionQuery) QueryMemo() *MemoQuery {
	query := (&MemoClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if query := sq.withMemo; query != nil {
		if err := sq.loadMemo(ctx, query, nodes, nil,
			func(n *Subscription, e *Memo) { n.Edges.Memo = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscriptionQuery) loadMemo(ctx context.Context, query *MemoQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Memo)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subscription)
	for i := range nodes {
		fk := nodes[i].MemoID
...
func (sq *SubscriptionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withSubscriber != nil {
			_spec.Node.AddColumnOnce(subscription.FieldUserID)
		}
		if sq.withMemo != nil {
			_spec.Node.AddColumnOnce(subscription.FieldMemoID)
		}
	}
